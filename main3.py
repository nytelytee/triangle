...
from example_drawers import ExampleRandomHPLuvColorDrawer
...
def main(timers: Subtimers) -> None:
    ...
    with timers.pushed('drawing'):
        with timers.pushed('setting up figure and drawers'):
            ...
            draw_a = ExampleRandomHPLuvColorDrawer(fig, ax, tree_a,
                                                   draw_zero=True)
            draw_b = ExampleRandomHPLuvColorDrawer(fig, ax, tree_b)
            draw_c = ExampleRandomHPLuvColorDrawer(fig, ax, tree_c)
    ...
    with timers.pushed('saving'):
        fig.savefig("output/output_random.png", pad_inches=0)
